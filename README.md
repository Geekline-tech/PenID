...
> **⚠️ 当前状态：UI 正在使用 PyQt5 + Fluent Widgets 重构中，版本不稳定，功能可能随时变动。**
...
| 识别页 | 训练页 | 人员管理 |
|:------:|:------:|:--------:|
| 上传图片 → Top-10 排行 | 实时训练日志 + 数据统计 | 特征库 CRUD |

## 技术架构

```
┌─────────────────────────────────────────────────────────┐
│           PyQt5 + Fluent Widgets (WinUI 3 风格)          │
│   识别页 │ 训练页 │ 人员管理页                             │
├─────────────────────────────────────────────────────────┤
│                    Inference Layer                       │
...
| 图像处理 | OpenCV | 中文路径兼容，背景减除 + CLAHE 增强 |
| UI 框架 | PyQt5 + PyQt-Fluent-Widgets | WinUI 3 风格，深色主题，侧边导航栏 |
...
│   ├── ui/
│   │   ├── app.py              # 主窗口 (FluentWindow)
│   │   └── pages/
│   │       ├── identify_page.py  # 识别页
│   │       ├── train_page.py     # 训练页
│   │       └── gallery_page.py   # 人员管理页
│   └── utils/
│       ├── config.py           # 全局配置
│       └── database.py         # SQLite
├── main.py                     # 入口
...
- [PyQt-Fluent-Widgets](https://github.com/ChinaIceF/PyQt-Fluent-Widgets) - WinUI 3 风格 UI 组件
